...
**Status**: ✅ **DONE — Verifier PASS** (12/12 ACs c/ evidência file:line; sensor 7/7 mutantes mortos incl. scope-bypass, raw-error leak, key-set, truncation, 401-guard, validação, length-bound; desvios B1–B5 confirmados OK; gate build+lint+277/277; ver `validation.md`). Histórico: Fase 1 ✅ COMPLETA (T1 `97c9490`, T2 `fe10f5e`, T3 `04fb6d1`, fix `827aa64`; build+lint+254/254 verdes; +15 testes). Fix loop 1 fechado: `InsertServerInput.purpose` opcional (read shapes intactos). Desvios aceitos: repository.test.ts já existia (estendido); regex de `classifyDomainError` estendida em error-middleware.ts (necessário pro AC de 400 sem violar camadas); fixture de omissão agora exercita omissão de verdade. **Fase 2 ✅ COMPLETA** (T4 `7cad898`, T5 `e838e75`; 275/275; +21 testes em discovery-tools.test.ts; ScopedMcp importado do domain; interfaces narrow locais — sem import do aggregator; SEC-10 por projeção de chaves + sanitização de throws, upstream isError proxiado verbatim; DISC-05 com assert de registry-nunca-chamado; tsc completo verde; 314 linhas aceitas — coeso por design, ~155 de código). **Fase 3 ✅ COMPLETA** (T6 `5742d64` cut-over + T7 `dbb1dbd`; 277/277 c/ build+lint verdes; delta: −8 aggregator, +9 novos [6 gateway-discovery, 3 secret-isolation], +1 spike). Desvios aceitos (Verifier DEVE confirmar): (1) server-assembly.test.ts fora da lista autorizada teve o E2E reescrito pro protocolo novo — cut-over quebrou o assert achatado; teste de 401 intocado; (2) reescrita do gateway-router.test.ts adiantada de T7→T6 pra manter cada commit verde. Ambos categoria "achatado superseded por AD-019"; todo assert 401/token preservado verbatim (MIG-01). **Fase 4 ✅ COMPLETA** (T8 `720cd56`, T9 `e855a4e`, T10 `6462b31`; build/build:web/lint verdes; 277/277 inalterado). Desvios aceitos: api-client.ts não precisou de mudança (encaminha input verbatim — verificado antes de decidir); label "Purpose" em inglês (UI toda em inglês; DESC-03 não manda idioma). **TODAS AS 10 TASKS FEITAS → Verifier (Opus) despachado.**
