...
- **Feature ativa: `gateway-discovery`** — Specify ✅ (12 IDs: DISC-01..07, DESC-01..03, SEC-10, MIG-01; usuário reforçou: escopo = matriz de atribuição por consumer, 0/alguns/todos MCPs). Design ✅ (abordagem A: `discovery-tools.ts` substitui `tool-aggregator.ts`, handlers trocados no gateway-router, migration 0002 `purpose`, sanitização de erro SEC-10; SDK `getInstructions()` verificado no 1.29.0; `pnpm build` já copia `*.sql` → carry-forward resolvido). **Aguardando aprovação do design → Tasks (4 fases propostas, 1 worker/fase + Verifier).**
