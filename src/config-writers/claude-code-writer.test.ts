...
import { writeConfig } from './claude-code-writer.js';
...
function readMcpJson(path: string): unknown {
  return JSON.parse(readFileSync(join(path, '.mcp.json'), 'utf-8'));
}
...
describe('claude-code-writer', () => {
  ...
  beforeEach(() => {
    projectDir = mkdtempSync(join(tmpdir(), 'mcp-manager-claude-code-'));
  });
  ...
  it('CFG-01: writes the managed entry with the correct url, headers and path', async () => {
  ...
  it('CFG-02: a second identical write is idempotent (status unchanged, no rewrite)', async () => {
    const consumer = buildConsumer(projectDir);

    await writeConfig(consumer, GATEWAY_BASE_URL, true);
    const beforeContent = readFileSync(join(projectDir, '.mcp.json'), 'utf-8');

    const second = await writeConfig(consumer, GATEWAY_BASE_URL, true);

    expect(second.status).toBe('unchanged');
    expect(readFileSync(join(projectDir, '.mcp.json'), 'utf-8')).toBe(beforeContent);
  });

  it('CFG-02: 0 assignments removes the managed entry (cleanup)', async () => {
    const consumer = buildConsumer(projectDir);
    await writeConfig(consumer, GATEWAY_BASE_URL, true);
