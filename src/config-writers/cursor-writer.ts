...
/** The shape of a Cursor `.cursor/mcp.json` file: `mcpServers` plus whatever
...
function configDir(consumer: ConsumerRecord): string {
  return join(consumer.path, '.cursor');
}
...
/**
 * CFG-C1..C4: writes a single managed `mcp-manager-gateway` entry
 * (type:http, url=`<gatewayBaseUrl>/mcp/<token>`, bearer auth header) into
 * the project's `.cursor/mcp.json`, preserving every other entry the user
 * already has. Creates the `.cursor` directory when it doesn't exist yet.
 * `hasAssignments=false` removes the managed entry instead (cleanup).
...
export const writeConfig: ConfigWriter['writeConfig'] = async (
  ...
  try {
    ...
    // Cleanup with nothing to clean is a no-op: never create a stub file or
    // reformat a user's file that doesn't contain our managed entry.
    if (!hasAssignments && (currentContent === undefined || !(MANAGED_KEY in existingServers))) {
      return { consumerId: consumer.id, format: 'cursor', path, status: 'unchanged' };
    }
